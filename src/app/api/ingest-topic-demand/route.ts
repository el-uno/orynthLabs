...
const ingestTopicDemandInputSchema = z.object({
  topic: z.string().min(3).max(120)
});

/** Queues topic-scoped demand ingestion. Poll GET /api/jobs/{jobRecordId}. */
export async function POST(request: Request) {
  ...
  const body = ingestTopicDemandInputSchema.safeParse(json);
  if (!body.success) {
  ...
  try {
    const jobRecordId = await recordJobQueued({
      ...
      jobType: "ingest-topic-demand",
      payload: { ...body.data }
    });

    const job = await queue.add("ingest-topic-demand", { ...body.data, jobRecordId });
