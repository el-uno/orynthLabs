...
const ingestConsumerInputSchema = z.object({
  owner: z.string().min(1),
  repo: z.string().min(1),
  windowDays: z.number().int().min(1).max(90).optional()
});

/** Queues consumer (user-voice) ingestion. Poll GET /api/jobs/{jobRecordId}. */
export async function POST(request: Request) {
  ...
  const body = ingestConsumerInputSchema.safeParse(json);
  if (!body.success) {
  ...
  try {
    const jobRecordId = await recordJobQueued({
      ...
      jobType: "ingest-consumer",
      payload: { ...body.data }
    });

    const job = await queue.add("ingest-consumer", { ...body.data, jobRecordId });
